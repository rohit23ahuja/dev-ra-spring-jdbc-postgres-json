...
public class ReleaseEventMapper implements RowMapper<ReleaseEvent> {
	...
	public ReleaseEvent mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if (rs.getTimestamp("execution_time", tzUTC) != null
				&& rs.getTimestamp("execution_time", tzUTC).toInstant() != null) {
			releaseEvent.setReleaseTime(rs.getTimestamp("execution_time", tzUTC).toInstant());
		}
